...
module.exports = {
    ...
    execute(params) {
        ...
        const sendInRoom = (user, data) => {
            ...
            userManager.send(usersInRoom.map((person) => person.id), `<sl>[p:<player_name>] says: ${data}<sl>`);
        };
        ...
        const sendLocally = (user, data) => {
            ...
            let usersInRoom = userManager.getRoomUsers(user.zoneId, user.roomId).filter((roomUser) => roomUser.id !== user.id) || [];
            // The message that other users in the room will see
            userManager.send(usersInRoom.map((person) => person.id), `<sl>[p:<player_name>] yells: ${data}<sl>`);
            ...
            _.forEach(exits, function (value) {
                ...
                userManager.send(usersInExit.map((person) => person.id), `<sl>(from nearby) [p:<player_name>] yells: ${data}<sl>`);
            });
        };
        ...
        const sendGlobally = (user, message) => {
            ...
            userManager.send(allUsers.map((person) => person.id), `<sl>[p:<player_name>] shouts: ${message}<sl>`);
        }
